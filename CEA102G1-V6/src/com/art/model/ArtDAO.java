...
public class ArtDAO implements ArtDAO_interface{
	...
	private static final String GETALLSE_STMT="SELECT * FROM ARTICLE WHERE ART_STATUS=1";
	...
	public List<String> getAllStatusEqualsOne() {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(GETALLSE_STMT);
			rs = pstmt.executeQuery();
			
			while(rs.next()) {
				String artNo = String.valueOf(rs.getInt("ART_NO"));
				list.add(artNo);
			}			
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. "
