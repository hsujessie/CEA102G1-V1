		...
		<div id="layoutSidenav_content">
			<main>
				<div class="container-fluid">
					<h2><c:if test='${funList.contains("管理員管理")}'>123</c:if></h2>
					<!-- error message Start -->
					...
					<div class="row " style="margin: -60px 0 20px 0;">
						...
						<div class="col-10">
							<jsp:useBean id="admSvc" scope="page"
								class="com.admin.model.AdmService" />
							<FORM class="form-sty" METHOD="post"
								ACTION="<%=request.getContextPath()%>/movie/mov.do">
								<b>電影名稱</b> <select name="mov_no" style="width: 80px;">
									<option value=""></option>
									<c:forEach var="movVO" items="${movSvcAll.all}">
										<option value="${movVO.movno}">${movVO.movname}
									</c:forEach>
								</select> <b>電影類型</b> <select name="mov_type">
									<option value=""></option>
								...
								</select> <b>選擇年份</b> <select name="mov_ondate_year">
									<option value=""></option>
									<c:forEach var="year" begin="1970"
										end="<%=(int) (java.util.Calendar.getInstance().get(java.util.Calendar.YEAR)) + 1%>">
										<option value="${year}">${year}年</option>
									</c:forEach>
								</select> <b>選擇月份</b> <select name="mov_ondate_month">
									<option value=""></option>
									<c:forEach var="month" begin="1" end="12">
										<option value="${month}">${month}月</option>
									</c:forEach>
								</select> <input type="hidden" name="action"
									value="listMovies_ByCompositeQuery"> <a
											...
											<td>
												<form method="post"
													...
													<a class="btn btn-light btn-brd grd1 effect-1"> <input
														type="hidden" name="admNo" value="${admVO.admNo}">
														<input type="hidden" name="action"
														value="getOne_for_update"> <input type="submit"
														value="修改" class="input-pos">
													</a>
												</form>
											</td>
										</tr>
